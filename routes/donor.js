...
router.get("/donor/viewcampaigns", middleware.ensureDonorLoggedIn, async (req,res) => {
	try
	{
		const currentCampaigns = await Campaign.find({}).populate("ngo");
		res.render("donor/viewcampaigns", { title: "Current Campaigns",currentCampaigns });
	}
	catch(err)
	{
		console.log(err);
